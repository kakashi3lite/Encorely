...
struct AIGeneratedMixtapeView: View {
    ...
    private var progressSection: some View {
        VStack(spacing: 20) {
            ...
            .accessibilityLabel("Generation progress")
            .accessibilityValue(
                "\(Int(viewModel.generationProgress * 100))% complete, \(viewModel.currentGenerationStep ?? "Processing...")")
            .accessibilityAddTraits(.updatesFrequently)
        }
    }
    ...
    private var advancedOptionsSection: some View {
        VStack(spacing: 16) {
            ...
            Toggle("Include explicit content", isOn: $viewModel.includeExplicitContent)
                ...
                .accessibilityHint(
                    "Double tap to \(viewModel.includeExplicitContent ? "exclude" : "include") explicit content")
                .padding(.horizontal)
...
class AIGeneratedMixtapeViewModel: ObservableObject {
    ...
    func saveSettings() {
        ...
        print(
            "Settings saved: Include Explicit Content - \(includeExplicitContent), Custom Mood Tag - \(customMoodTag)")
    }
}
