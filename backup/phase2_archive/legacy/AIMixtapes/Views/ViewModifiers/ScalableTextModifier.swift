...
struct MaxFontSizeModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.transformEffect(.init(
            scaleX: min(1, maxSize / UIFont.preferredFont(forTextStyle: .body).pointSize),
            y: min(1, maxSize / UIFont.preferredFont(forTextStyle: .body).pointSize)
        ))
    }
}
