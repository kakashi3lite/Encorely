...
struct AssetValidator {
    ...
    func validateAssets() -> ValidationResult {
        ...
        guard let manifestData = try? Data(contentsOf: URL(fileURLWithPath: manifestPath)),
              let manifest = try? JSONDecoder().decode(AssetManifestGenerator.Manifest.self, from: manifestData)
        else {
            ...
            return ValidationResult(
                ...
                missingAssets: [],
                extraAssets: [],
                invalidAssets: ["Invalid manifest"]
            )
        }
        ...
        let currentAssetNames = Set(currentAssets.map(\.name))

        // Extract names from manifest
        let manifestAssetNames = Set(manifest.assets.map(\.name))

        // Find missing and extra assets
        let requiredManifestAssets = manifest.assets.filter(\.requiredForApp)
        let requiredAssetNames = Set(requiredManifestAssets.map(\.name))
        ...
        for currentAsset in currentAssets {
            if let manifestAsset = manifest.assets.first(where: { $0.name == currentAsset.name }) {
                if currentAsset.type != manifestAsset.type {
                    invalidAssets
                        .append("\(currentAsset.name) (expected: \(manifestAsset.type), found: \(currentAsset.type))")
                }
            }
        }
