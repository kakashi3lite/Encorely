...
class FFTProcessor {
    ...
    init(maxFrameSize: Int, sampleRate: Float = 44100.0) {
        ...
        tempoEstimator = TempoEstimator(bufferSize: maxFrameSize)
    }
    ...
    private func calculateBandEnergies(magnitudes: [Float],
                                       frequencyResolution: Float) -> (bass: Float, mid: Float, treble: Float)
    {
...
struct TempoEstimator {
    ...
    init(bufferSize: Int) {
        tempBuffer = [Float](repeating: 0, count: bufferSize)
    }
    ...
    private func calculateSmoothedTempo() -> Float {
        ...
        for tempo in tempoHistory.dropFirst() {
            smoothedTempo = smoothedTempo * (1 - TempoEstimator.smoothingFactor) + tempo * TempoEstimator
                .smoothingFactor
        }
