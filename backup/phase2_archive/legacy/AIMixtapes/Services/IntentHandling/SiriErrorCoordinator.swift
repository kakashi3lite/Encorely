...
final class SiriErrorCoordinator {
    ...
    private func determineRecoveryAction(for error: Error, intent: INIntent) -> (() -> AnyPublisher<Bool, Never>)? {
        ...
        case let intentError as IntentError:
            { self.handleIntentError(intentError, for: intent) }
        case let appError as AppError:
            { self.handleAppError(appError, for: intent) }
        default:
            nil
        }
    }
    ...
    private func handleAppError(_: AppError, for _: INIntent) -> AnyPublisher<Bool, Never> {
        Future { promise in
    ...
    private func repairInvalidParameters(_: INIntent) -> AnyPublisher<Bool, Never> {
        Future { promise in
    ...
    private func handleTimeout(_: INIntent) -> AnyPublisher<Bool, Never> {
        Future { promise in
