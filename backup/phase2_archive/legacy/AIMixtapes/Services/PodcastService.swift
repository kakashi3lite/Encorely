...
public class PodcastService {
    ...
    public func transcribeAudio(from url: URL) async throws -> String {
        ...
        let recognitionTask = speechRecognizer?.recognitionTask(with: recognitionRequest!) { _, _ in
            // Not used—see below
        }
        ...
        var finalText = ""
        if let task = speechRecognizer?.recognitionTask(with: recognitionRequest!) { result, _ in
