...
final class PerformanceTests: XCTestCase {
    ...
    func testAudioProcessingPerformance() async throws {
        ...
        Task {
            ...
            print(
                "Latency: \(results.latencyResult.passed ? "PASSED" : "FAILED") - Avg: \(String(format: "%.1f", results.latencyResult.averageLatencyMs))ms")
            print(
                "Memory: \(results.memoryResult.passed ? "PASSED" : "FAILED") - Peak: \(String(format: "%.1f", results.memoryResult.peakMemoryMB))MB")
            print(
                "Accuracy: \(results.accuracyResult.passed ? "PASSED" : "FAILED") - \(String(format: "%.1f", results.accuracyResult.accuracy * 100))%")
