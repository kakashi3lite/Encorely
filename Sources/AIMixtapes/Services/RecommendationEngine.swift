...
class RecommendationEngine: ObservableObject {
    ...
    private func extractFeatures(from songs: [Song]) async throws -> [AudioFeatures] {
        return try await withThrowingTaskGroup(of: AudioFeatures?.self) { group in
    ...
    private func filterByMood(_ features: [AudioFeatures], mood: Mood) -> [AudioFeatures] {
        let filtered = features.filter { feature in
            switch mood {
                case .energetic:
                    return feature.energy > 0.7 && feature.tempo > 120
                case .relaxed:
                    return feature.energy < 0.4 && feature.valence > 0.4
                case .happy:
                    return feature.valence > 0.7
                case .melancholic:
                    return feature.valence < 0.4 && feature.tempo < 100
                case .focused:
                    return feature.complexity > 0.6 && feature.instrumentalness > 0.7
                case .romantic:
                    return feature.valence > 0.6 && feature.energy < 0.6
                case .angry:
                    return feature.energy > 0.8 && feature.valence < 0.4
                case .neutral:
                    return true
            }
        }
