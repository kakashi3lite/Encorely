...
extension Mood {
    /// Representative color for UI elements
    var color: Color {
        switch self {
...
public final class MoodEngine: ObservableObject {
    ...
    private func detectMoodFromFeatures(_ features: AudioFeatures) -> (mood: Mood, confidence: Float) {
        ...
        guard let top = moodScores.first else { return (.neutral, 0.5) }
        let secondScore = moodScores.dropFirst().first?.score ?? 0
        // Derive a confidence value from margin between top two scores
        let margin = max(0, top.score - secondScore)
        let confidence = min(1.0, 0.5 + margin) // baseline 0.5 + margin
        return (top.mood, confidence)
    }
