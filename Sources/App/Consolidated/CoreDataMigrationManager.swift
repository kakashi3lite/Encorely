...
class CoreDataMigrationManager: NSObject {
    ...
    private let mappingModelURL: URL?

    init(storeURL: URL) {
        self.storeURL = storeURL
        backupStoreURL = storeURL.deletingLastPathComponent()
            .appendingPathComponent("AI_Mixtapes.backup.sqlite")

        // Initialize mapping model URL
        mappingModelURL = Bundle.main.url(
            forResource: "AI_Mixtapes_v1_to_v2",
            withExtension: "xcmappingmodel"
        )
    }
    ...
    func migrateStore() -> Bool {
        ...
        // Verify store exists and is readable
        guard FileManager.default.isReadableFile(atPath: storeURL.path) else {
            logger.error("Store file is not readable at \(storeURL.path)")
            return false
        }
        ...
        do {
            ...
            guard let sourceVersion = try determineSourceVersion(),
                  let destinationVersion = modelVersions.last
            else {
            ...
            guard modelVersions.contains(sourceVersion),
                  modelVersions.contains(destinationVersion),
                  sourceVersion != destinationVersion
            else {
                logger.error("Invalid migration path: \(sourceVersion) -> \(destinationVersion)")
                return false
            }
            ...
            return handleMigrationError(error, sourceVersion: "", destinationVersion: "")
        }
    }
    ...
    private func migrateFromV1ToV2() throws -> Bool {
        guard let sourceModel = managedObjectModel(forVersion: "AI_Mixtapes"),
              let destinationModel = managedObjectModel(forVersion: "AI_Mixtapes_v2")
        else {
        ...
        if let mappingModelURL {
            mappingModel = try? NSMappingModel(contentsOf: mappingModelURL)
        }

        // Fall back to inferred mapping model if needed
        if mappingModel == nil {
            logger.info("No custom mapping model found, attempting to infer mapping")
        ...
        guard let mappingModel else {
            logger.error("Failed to create or infer mapping model")
            return false
        }
    ...
    private func performIncrementalMigration(from sourceVersion: String) throws -> Bool {
        guard let sourceVersionIndex = modelVersions.firstIndex(of: sourceVersion),
              sourceVersionIndex < modelVersions.count - 1
        else {
    ...
    private func validateMigrationProgress(_ manager: NSMigrationManager) -> Bool {
        let startTime = Date()
        var lastProgress: Float = 0

        repeat {
            let currentProgress = manager.migrationProgress
            if currentProgress > lastProgress {
                lastProgress = currentProgress
                logger.info("Migration progress: \(Int(currentProgress * 100))%")
            }

            if Date().timeIntervalSince(startTime) > migrationTimeout {
                logger.error("Migration timed out after \(migrationTimeout) seconds")
                return false
            }

            Thread.sleep(forTimeInterval: 0.1)
        } while manager.migrationProgress < 1.0

        return true
    }

    private func handleMigrationError(_ error: Error, sourceVersion: String, destinationVersion: String) -> Bool {
        logger.error("""
            Migration failed from \(sourceVersion) to \(destinationVersion)
