...
enum MLConfig {
    ...
    static func defaultConfiguration() -> MLModelConfiguration {
        ...
        config.computeUnits = Performance.computeUnits
        config.preferredMetalDevice = MTLCreateSystemDefaultDevice()
        config.allowLowPrecisionAccumulationOnGPU = Performance.allowLowPrecisionAccumulationOnGPU
        return config
    }
    ...
    enum ModelAsset: String {
        ...
        var url: URL? {
            ...
            if self == .emotionClassifier,
               let optimizedUrl = Bundle.main.url(
                   forResource: ModelAsset.emotionClassifierOptimized.rawValue,
    ...
    enum ResourceManagement {
        ...
        static func shouldUnloadModel(lastUsed: Date) -> Bool {
            Date().timeIntervalSince(lastUsed) > modelUnloadThreshold
        }
    }
