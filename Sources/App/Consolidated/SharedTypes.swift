...
class RecommendationEngine: ObservableObject {
    ...
    func updateMood(_ mood: Mood) { currentMood = mood
        invalidateCache()
    }

    func updatePersonality(_ personality: PersonalityType) { currentPersonality = personality
        invalidateCache()
    }
    ...
    private func filterByPersonality(
        ...
        switch personality {
        case .explorer:
            features.shuffled().map { ($0.song, $0) }
        case .enthusiast:
            features.filter { $0.energy > 0.6 || $0.danceability > 0.6 }.map { ($0.song, $0) }
        case .curator, .planner:
            features
                .filter { (0.4 ... 0.7).contains(Double($0.valence)) && (0.3 ... 0.7).contains(Double($0.danceability))
                }.map { (
                    $0.song,
                    $0
                ) }
        case .creative:
            features.filter { $0.instrumentalness > 0.5 || $0.liveness > 0.5 }.map { ($0.song, $0) }
        case .ambient:
            features.filter { $0.energy < 0.5 && $0.speechiness < 0.5 }.map { ($0.song, $0) }
        case .balanced, .neutral, .analyzer, .social:
            features.map { ($0.song, $0) }
        }
    }
    ...
    private func scoreByMood(
        ...
        features.map { pair in
            ...
            if let w = weights {
                ...
                let hybrid = 0.7 * base + 0.3 *
                    (0.35 * energyAdj + 0.25 * complexityAdj + 0.25 * tempoAdj + 0.15 * acousticAdj)
