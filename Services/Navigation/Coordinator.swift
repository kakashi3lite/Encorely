...
class AppCoordinator: NavigationCoordinator {
    ...
    func navigate(to destination: NavigationDestination) {
        switch destination {
        case .mixtapeDetail(let mixtape):
    ...
    func handleDeepLink(_ url: URL) {
        ...
        case "mixtape":
            if let idString = components?.path.dropFirst().description,
               ...
               let coordinator = container.persistenceController.container.persistentStoreCoordinator.managedObjectID(forURIRepresentation: objectURL) {
...
extension AppCoordinator {
    static let preview: AppCoordinator = {
        AppCoordinator(container: .preview)
    }()
}
#endif
